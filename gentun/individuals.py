...
from .models.keras_models2 import GeneticLSTMModel
...
class GeneticLSTMIndividual(Individual):

    def __init__(self, x_train, y_train, genome=None, genes=None, crossover_rate=0.3, mutation_rate=0.1, nodes=(3, 5),
                 input_shape=(28, 28, 1), kernels_per_layer=(80, 30), dense_units=500,
                 dropout_probability=0.2, last_lstm_units=10,
        ...
        if genes is None:
            genes = self.generate_random_genes(genome)
        # Set individual's attributes
        super(GeneticLSTMIndividual, self).__init__(x_train, y_train, genome, genes, crossover_rate, mutation_rate)
        # Set additional parameters which are not tuned
        assert len(nodes) == len(kernels_per_layer)
        self.nodes = nodes
        self.input_shape = input_shape
        self.kernels_per_layer = kernels_per_layer
        self.dense_units = dense_units
        self.last_lstm_units = last_lstm_units
        self.dropout_probability = dropout_probability
        self.kfold = kfold
        self.epochs = epochs
        self.learning_rate = learning_rate
    ...
    def evaluate_fitness(self):
        """Create model and perform cross-validation."""
        model = GeneticLSTMModel(
            self.x_train, self.y_train, self.genes, self.nodes, self.input_shape, self.kernels_per_layer,
            self.dense_units, self.dropout_probability, self.last_lstm_units,
            self.kfold, self.epochs, self.learning_rate, self.batch_size
        )
        self.fitness = model.cross_validate()
    ...
    def get_additional_parameters(self):
        return {
            ...
            'input_shape': self.input_shape,
            'kernels_per_layer': self.kernels_per_layer,
            'last_lstm_units': self.last_lstm_units,
            'dense_units': self.dense_units,
            'dropout_probability': self.dropout_probability,
            'classes': self.classes,
            'kfold': self.kfold,
